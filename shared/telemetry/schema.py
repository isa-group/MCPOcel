...
class ApiCallEvent(BaseModel):
    ...
    run_id:      str = Field(
        default="",
        description=(
            "Pipeline run identifier. Groups together all events "
            "from the same extraction to answer the question: 'How much did this"
            "run cost?'. Automatically generated by InstrumentedSession if "
            "not provided (compact hex uuid4)."
        ),
    )
    ...
    def _infer_throttle_from_status(self) -> "ApiCallEvent":
        if self.status_code == 429:
            self.throttle_signal = ThrottleSignal(
                was_throttled=True,
                retry_after_seconds=self.throttle_signal.retry_after_seconds,
                was_cached=self.throttle_signal.was_cached,
            )
        if self.status_code == 304:
            self.throttle_signal = ThrottleSignal(
                was_throttled=self.throttle_signal.was_throttled,
                retry_after_seconds=self.throttle_signal.retry_after_seconds,
                was_cached=True,
            )
        return self
